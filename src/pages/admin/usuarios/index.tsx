...
export const UsuariosPage: React.FC = () => {
  ...
  // Permissões
  const canRead = autenticado && usuario.permissoes.some(
    (p) => p.modulo === ModuloSistema.USUARIOS_PERMISSOES && p.acao === AcaoPermissao.LER && p.permitido
  );
  const canCreate = autenticado && usuario.permissoes.some(
    (p) => p.modulo === ModuloSistema.USUARIOS_PERMISSOES && p.acao === AcaoPermissao.CRIAR && p.permitido
  );
  const canEdit = autenticado && usuario.permissoes.some(
    (p) => p.modulo === ModuloSistema.USUARIOS_PERMISSOES && p.acao === AcaoPermissao.EDITAR && p.permitido
  );
  const canDelete = autenticado && usuario.permissoes.some(
    (p) => p.modulo === ModuloSistema.USUARIOS_PERMISSOES && p.acao === AcaoPermissao.DELETAR && p.permitido
  );
