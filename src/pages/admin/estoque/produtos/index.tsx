...
const ProdutosPage: React.FC = () => {
  ...
  const filteredProdutos = produtos?.filter((produto: Produto) => {
    ...
    const matchesTab = currentTab === 'all' || 
                      (currentTab === 'insumos' && ['INSUMO', 'MATERIA_PRIMA', 'PRINCIPIO_ATIVO', 'EXCIPIENTE'].includes(produto.tipo)) ||
                      (currentTab === 'embalagens' && produto.tipo === 'EMBALAGEM') ||
                      (currentTab === 'cosmeticos' && produto.tipo === 'COSMÉTICO') ||
                      (currentTab === 'medicamentos' && produto.tipo === 'MEDICAMENTO');
  ...
  const totalInsumos = produtos?.filter((p: Produto) => ['INSUMO', 'MATERIA_PRIMA', 'PRINCIPIO_ATIVO', 'EXCIPIENTE'].includes(p.tipo)).length || 0;
  const totalEmbalagens = produtos?.filter((p: Produto) => p.tipo === 'EMBALAGEM').length || 0;
  const totalCosmeticos = produtos?.filter((p: Produto) => p.tipo === 'COSMÉTICO').length || 0;
        ...
        <div className="px-6 w-full">
          <Tabs value={currentTab} onValueChange={setCurrentTab} className="w-full">
            <TabsList className="grid w-full grid-cols-5">
              ...
              <TabsTrigger value="insumos">Insumos ({totalInsumos})</TabsTrigger>
              <TabsTrigger value="embalagens">Embalagens ({totalEmbalagens})</TabsTrigger>
              <TabsTrigger value="cosmeticos">Cosméticos ({totalCosmeticos})</TabsTrigger>
                              ...
                              <TableRow key={produto.id}>
                                ...
                                <TableCell>
                                  <span className="text-sm">
                                    {produto.fornecedor_nome || produto.fornecedores?.nome || 'Não informado'}
                                  </span>
                                </TableCell>
                                <TableCell>
